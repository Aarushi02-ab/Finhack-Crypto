...
def write(master, name, oc, pin):
    ...
    f1.close()
    with open("Account_record.csv", 'a+', newline='') as obj:
        fdet = csv.writer(obj)
        ...
        while True:
            ...
            Opening_Credit =oc
            Pin=pin
            record = [Account_no,Name, Opening_Credit,Pin]
            fdet.writerow(record)
            break;
...
def logged_in_menu(accnt, name):
    ...
    rootwn.configure(background='#000080')
    fr1 = tk.Frame(rootwn)
    fr1.pack(side="top")
    l_title = tk.Message(rootwn, text=" BANKING\n APPLICATION", relief="raised", width=2000, padx=600, pady=0,
...
def Main_Menu():
    ...
    fr1 = tk.Frame(rootwn)
    fr1.pack(side="top")
    bg_image = ImageTk.PhotoImage(file="images/login_image2.jpg")
    x = tk.Label(image=bg_image)
    x.place(x=50,y=10)
    l_title = tk.Message(text=" BANKING APPLICATION\n", relief="raised", width=2000, padx=600, pady=0, fg="white",
                         bg="black", justify="center", anchor="center")
    l_title.config(font=("Courier", "50", "bold"))
    l_title.pack(side="top")
...
class Login:
    def __init__(self, root):
        ...
        self.bg = ImageTk.PhotoImage(file="images/Login_image.jpeg")
        ...
        title1 = Label(Frame_login, text="United Bank", font=("Impact", 30, "bold"), fg="#d77337", bg='white').place(x=90,y=30)
        desc = Label(Frame_login, text="Login Here", font=("Goudy old style", 10, "bold"), fg="gray", bg='white').place( x=90, y=100)
        customer_Id = Label(Frame_login, text="Customer ID", font=("Goudy old style", 10, "bold"), fg="gray", bg='white').place(x=90,y=140)
        self.text_user = Entry(Frame_login,bg="lightgray")
        self.text_user.place(x=90,y=170,width=350,height=35)
        password = Label(Frame_login, text="Password", font=("Goudy old style", 10, "bold"), fg="gray", bg='white').place(x=90, y=210)
        self.text_pass=Entry(Frame_login,show="*",bg="lightgray")
        self.text_pass.place(x=90, y=240, width=350, height=35)
        Login_btn=Button(Frame_login,text="Login",bg="white",fg="#d77337",font=("times new roman",15)).place(x=90,y=280)
...
def write(master, name, oc, pin):
    ...
    tkinter.messagebox.showinfo("Details", "Your Account Number is:" + str(accnt_no))
    master.destroy()
    return
def is_number(s):
    try:
...
def Create():
    ...
    fr1 = tkinter.Frame(crwn, bg="blue")
    l_title = tkinter.Message(crwn, text="UNITED BANK", relief="raised", width=2000, padx=600, pady=0, fg="white",
                         bg="black", justify="center", anchor="center")
    l_title.config(font=("Courier", "50", "bold"))
    l_title.pack(side="top")
    l1 = tkinter.Label(crwn, text="Enter Name:", relief="raised")
    l1.pack(side="top")
    e1 = tkinter.Entry(crwn)
    e1.pack(side="top")
    l2 = tkinter.Label(crwn, text="Enter opening credit:", relief="raised")
    l2.pack(side="top")
    e2 = tkinter.Entry(crwn)
    e2.pack(side="top")
    l3 = tkinter.Label(crwn, text="Enter desired PIN:", relief="raised")
    l3.pack(side="top")
    e3 = tkinter.Entry(crwn, show="*")
    e3.pack(side="top")
    b = tkinter.Button(crwn, text="Submit",
                  command=lambda: write(crwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))
    b.pack(side="top")
    crwn.bind("<Return>", lambda x: write(crwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))"""
